...
class AssetLibrary:
    ...
    def load(self) -> None:
        ...
        assets_and_roots += [
            ...
            for name in (
                ASSET_DICT["roads"]["straight"]
                + ASSET_DICT["roads"]["crossroad"]
                + ASSET_DICT["roads"]["bend"]
                + ASSET_DICT["roads"]["intersection"]
            )
        ]
...
class CityGenerator:
    ...
    def run(self) -> None:
        ...
        road_component_ratio = self._largest_road_component_ratio()
        print(
            "[DONE] "
            ...
            f"industrial_detail={self.stats['industrial_detail']}, road_component_ratio={road_component_ratio:.3f}"
        )
    ...
    def generate_roads(self) -> None:
        ...
        verticals = self._choose_irregular_lines(4, GRID_SIZE - 5, (3, 5), 4)
        horizontals = self._choose_irregular_lines(4, GRID_SIZE - 5, (3, 5), 4)

        for x in verticals:
            self._carve_road_line("x", x)
        for y in horizontals:
            self._carve_road_line("y", y)

        protected_nodes = self._collect_intersections(verticals, horizontals)
        max_attempts = 30
        target_success = self.rng.randint(8, 14)
        success_count = 0
        ...
        for _ in range(max_attempts):
            ...
            if axis == "x":
                candidates = [x for x in verticals if x not in (border, GRID_SIZE - 1 - border)]
                if not candidates:
                    continue
                line_index = self.rng.choice(candidates)
                anchors = sorted([border, GRID_SIZE - 1 - border] + list(horizontals))
            else:
                candidates = [y for y in horizontals if y not in (border, GRID_SIZE - 1 - border)]
                if not candidates:
                    continue
                line_index = self.rng.choice(candidates)
                anchors = sorted([border, GRID_SIZE - 1 - border] + list(verticals))

            if self._try_remove_road_segment(axis, line_index, anchors, protected_nodes):
                success_count += 1

    def _choose_irregular_lines(
        self, min_pos: int, max_pos: int, count_range: Tuple[int, int], min_gap: int
    ) -> List[int]:
        candidates = list(range(min_pos, max_pos + 1))
        self.rng.shuffle(candidates)
        target_count = self.rng.randint(count_range[0], count_range[1])
        chosen: List[int] = []

        for pos in candidates:
            if all(abs(pos - existing) >= min_gap for existing in chosen):
                chosen.append(pos)
            if len(chosen) >= target_count:
                break

        if not chosen:
            chosen.append((min_pos + max_pos) // 2)
        return sorted(chosen)

    def _carve_road_line(self, axis: str, index: int) -> None:
        if axis == "x":
            for y in range(1, GRID_SIZE - 1):
                self.grid.set_force(index, y, ROAD)
        else:
            for x in range(1, GRID_SIZE - 1):
                self.grid.set_force(x, index, ROAD)

    def _collect_intersections(
        self, verticals: Sequence[int], horizontals: Sequence[int]
    ) -> Set[Tuple[int, int]]:
        ...
        for x in verticals:
            nodes.add((x, border))
            nodes.add((x, GRID_SIZE - 1 - border))
        for y in horizontals:
            nodes.add((border, y))
            nodes.add((GRID_SIZE - 1 - border, y))
        return nodes

    def _try_remove_road_segment(
        self,
        axis: str,
        line_index: int,
        anchors: Sequence[int],
        protected_nodes: Set[Tuple[int, int]],
    ) -> bool:
        if len(anchors) < 2:
            return False
        ...
        for start_anchor, end_anchor in spans:
            available_start = start_anchor + 1
            available_end = end_anchor - 1
            available_len = available_end - available_start + 1
            if available_len < 2:
                continue

            seg_len = min(self.rng.randint(2, 5), available_len)
            start = self.rng.randint(available_start, available_end - seg_len + 1)
            cells = []
            blocked = False
            for offset in range(seg_len):
                moving = start + offset
                cell = (line_index, moving) if axis == "x" else (moving, line_index)
                if cell in protected_nodes:
                    blocked = True
                    break
                cells.append(cell)
            if blocked:
            ...
            backup = {cell: self.grid.get(cell[0], cell[1]) for cell in cells}
            for cx, cy in cells:
                self.grid.set_force(cx, cy, EMPTY)

            if self._largest_road_component_ratio() == 1.0:
                return True

            for (cx, cy), value in backup.items():
                if value is not None:
                    self.grid.set_force(cx, cy, value)

        return False

    def _largest_road_component_ratio(self) -> float:
        road_cells = {(x, y) for x, y in self.grid.iter_cells() if self.grid.get(x, y) == ROAD}
        if not road_cells:
            return 0.0

        remaining = set(road_cells)
        largest = 0
        while remaining:
        ...
        return largest / float(len(road_cells))

    def plan_roads_instances(self) -> None:
        for x, y in self.grid.iter_cells():
            if self.grid.get(x, y) != ROAD:
                continue
            neighbors = set(self.grid.road_neighbors(x, y).keys())
            asset_name, rot = self._road_asset_from_neighbors(neighbors)
            self.road_plan.append((x, y, asset_name, rot))

    def _road_asset_from_neighbors(self, neighbors: Set[str]) -> Tuple[str, float]:
        count = len(neighbors)
        if count >= 4:
            return "road-crossroad", 0.0
        if count == 3:
            missing = next(direction for direction in ("N", "E", "S", "W") if direction not in neighbors)
            rot = {"S": 0.0, "W": math.pi * 0.5, "N": math.pi, "E": math.pi * 1.5}[missing]
            return "road-intersection", rot
        if count == 2:
            if neighbors in ({"E", "W"}, {"N", "S"}):
                rot = 0.0 if neighbors == {"E", "W"} else math.pi * 0.5
                return "road-straight", rot
            rot_map = {
                frozenset({"N", "E"}): 0.0,
            ...
            return "road-bend", rot_map[frozenset(neighbors)]
        if count == 1:
            direction = next(iter(neighbors))
            rot = 0.0 if direction in ("E", "W") else math.pi * 0.5
            return "road-straight", rot
        return "road-straight", 0.0
    ...
    def _is_roadside_or_near_road(self, x: int, y: int) -> bool:
        ...
        for dx, dy in dirs.values():
            if self.grid.get(x + dx * 2, y + dy * 2) == ROAD:
                return True
            if self.grid.get(x + dx * 3, y + dy * 3) == ROAD:
                return True
        return False
