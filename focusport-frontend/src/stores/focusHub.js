...
const normalizeTaskFocusMinutes = (value = {}) => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {}
  return Object.entries(value).reduce((acc, [taskId, minutes]) => {
    const normalizedId = String(taskId || '').trim()
    const normalizedMinutes = Number(minutes)
    if (normalizedId && Number.isFinite(normalizedMinutes) && normalizedMinutes > 0) {
      acc[normalizedId] = Math.round(normalizedMinutes)
    }
    return acc
  }, {})
}
...
const normalizeState = (value = {}) => ({
  ...
  taskFocusMinutes: normalizeTaskFocusMinutes(value.taskFocusMinutes),
...
export const useFocusHubStore = defineStore('focusHub', () => {
  ...
  const persist = () => {
    ...
    window.localStorage.setItem(storageKey(hydratedUsername.value), JSON.stringify({
      ...
      taskFocusMinutes: taskFocusMinutes.value,
  ...
  const hydrate = (username = currentUsername()) => {
    ...
    taskFocusMinutes.value = normalized.taskFocusMinutes
  ...
  const getTaskFocusMinutes = (taskId) => {
    const normalizedId = String(taskId || '').trim()
    const minutes = Number(taskFocusMinutes.value[normalizedId])
    return Number.isFinite(minutes) && minutes > 0 ? minutes : 25
  }

  const setTaskFocusMinutes = (taskId, minutes) => {
    if (pomodoro.value.isRunning) return
    const normalizedId = String(taskId || '').trim()
    if (!normalizedId) return
    const mins = Number(minutes) > 0 ? Number(minutes) : 25
    taskFocusMinutes.value = {
      ...taskFocusMinutes.value,
      [normalizedId]: Math.round(mins)
    }
  ...
  watch([lastAccessDate, pomodoro, taskFocusMinutes, todayTasks, archiveTasks, weekNodes, countdowns], () => {
    debouncedPersist()
  }, { deep: true })
