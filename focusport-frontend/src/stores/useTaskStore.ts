...
const storageKey = (username = currentUsername()): string => `${STORAGE_KEY}.${username || 'guest'}`
...
const loadSnapshot = (username = currentUsername()): Snapshot => {
  ...
  const raw = safeParse(
    ...
    (username === 'guest' ? null : window.localStorage.getItem(STORAGE_KEY))
  ) as any
...
export const useTaskStore = defineStore('taskStore', () => {
  const hydratedUsername = ref<string>(currentUsername())
  const snapshot = loadSnapshot(hydratedUsername.value)
  ...
  const hydrateForUser = (username = currentUsername()): void => {
    const nextUsername = username || 'guest'
    if (hydratedUsername.value === nextUsername) return
    const nextSnapshot = loadSnapshot(nextUsername)
    hydratedUsername.value = nextUsername
    tasks.value = nextSnapshot.tasks
    sessionLogs.value = nextSnapshot.sessionLogs
    taskCompletionLogs.value = nextSnapshot.taskCompletionLogs
    taskRewardLogs.value = nextSnapshot.taskRewardLogs
    wallet.value = nextSnapshot.wallet
    analysisStateByTaskId.value = nextSnapshot.analysisStateByTaskId
    analysisResultByTaskId.value = nextSnapshot.analysisResultByTaskId
    lastRolloverDateKey.value = nextSnapshot.lastRolloverDateKey
    lastSeenDateKey.value = nextSnapshot.lastSeenDateKey
    pendingAnalysisByTaskId.clear()
