...
const loadCachedTexture = async (texturePath) => {
  if (!texturePath) return null
  const encodedPath = texturePath.replace(/ /g, '%20')
  if (!textureCache.has(encodedPath)) {
    textureCache.set(encodedPath, new THREE.TextureLoader().loadAsync(encodedPath))
  }
  const texture = await textureCache.get(encodedPath)
  texture.colorSpace = THREE.SRGBColorSpace
  texture.needsUpdate = true
  return texture
}

const loadCachedSourceModel = async (preferredPath) => {
  const encodedPath = preferredPath.replace(/ /g, '%20')
  if (!modelSourceCache.has(encodedPath)) {
    const loading = encodedPath.toLowerCase().endsWith('.obj')
      ? loadObjModel(encodedPath)
      : new GLTFLoader().loadAsync(encodedPath).then((gltf) => {
          if (!gltf?.scene) {
            throw new Error(`GLTF scene is empty: ${encodedPath}`)
          }
          return gltf.scene
        })
    modelSourceCache.set(encodedPath, loading)
  }
  return modelSourceCache.get(encodedPath)
}
...
const loadModel = async () => {
  ...
  try {
    ...
    if (texturePath) {
      sharedTexture = await loadCachedTexture(texturePath)
    }
    const sourceScene = await loadCachedSourceModel(preferredPath)
    const sceneObject = sourceScene.clone(true)
    ...
    if (preferredPath !== modelPath.value && /\.obj$/i.test(fallbackObjPath)) {
      try {
        const fallbackScene = (await loadCachedSourceModel(modelPath.value)).clone(true)
        if (currentSequence !== loadSequence.value) return
