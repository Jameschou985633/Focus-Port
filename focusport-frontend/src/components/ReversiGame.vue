...
const pendingOnlinePass = ref(false)
let ws = null
...
function placePiece(row, col) {
  ...
  if (isOnline.value && pendingOnlinePass.value) return
...
function settleOnlineTurn() {
  if (!isOnline.value || gameOver.value || !onlineSynced.value || !ws || ws.readyState !== WebSocket.OPEN) return
  const currentHasMove = hasAnyValidMove(currentTurn.value)
  const otherColor = currentTurn.value === 1 ? 2 : 1
  const otherHasMove = hasAnyValidMove(otherColor)
  if (!currentHasMove && !otherHasMove) {
    const black = scores.value.black
    const white = scores.value.white
    ws.send(JSON.stringify({
      type: 'game_over',
      winner_color: black === white ? 0 : (black > white ? 1 : 2),
      winner_name: ''
    }))
    return
  }
  if (isMyTurn.value && !currentHasMove && !pendingOnlinePass.value) {
    pendingOnlinePass.value = true
    ws.send(JSON.stringify({ type: 'pass', username: username.value }))
  }
}
