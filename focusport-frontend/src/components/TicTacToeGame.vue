...
const pendingOnlineMove = ref(false)
let ws = null

const username = computed(() => userStore.username || 'Player')
const onlineRoleLabel = computed(() => {
  if (!isOnline.value) return ''
  if (!onlineSynced.value || !onlineMyColor.value) return '同步房间中...'
  return onlineMyColor.value === 1 ? '你是 X（房主先手）' : '你是 O（等待房主先手）'
})
...
function handleWsMessage(msg) {
  if (msg.type === 'sync') {
    onlineSynced.value = true
    pendingOnlineMove.value = false
    board.value = Array.from({ length: SIZE }, () => Array(SIZE).fill(0))
    gameOver.value = msg.status === 'finished' || Boolean(msg.winner || msg.is_draw)
    winner.value = Number(msg.winner || 0)
    isDraw.value = Boolean(msg.is_draw)
    // Restore state from sync
    if (msg.player_host === username.value) {
      onlineMyColor.value = 1
    } else if (msg.player_guest === username.value) {
    ...
    if (msg.moves) {
      for (const m of msg.moves) {
        if (m.row >= 0 && m.row < SIZE && m.col >= 0 && m.col < SIZE && board.value[m.row][m.col] === 0) {
          board.value[m.row][m.col] = m.color
        }
      }
      const last = msg.last_move || msg.moves[msg.moves.length - 1]
      if (!gameOver.value && last && checkWin(last.row, last.col, last.color)) {
        gameOver.value = true
        winner.value = last.color
        if (ws && ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({
            type: 'game_over',
            winner_color: last.color,
            winner_name: last.username || ''
          }))
        }
      } else if (!gameOver.value && board.value.flat().every(v => v !== 0)) {
        gameOver.value = true
        isDraw.value = true
        if (ws && ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({ type: 'game_over', winner_color: 0, winner_name: '' }))
        }
      }
    }
    currentTurn.value = Number(msg.current_turn || 1)
...
function placeStone(r, c) {
  ...
  if (isOnline.value && !onlineSynced.value) return
  if (isOnline.value && !onlineMyColor.value) return
  if (isOnline.value && pendingOnlineMove.value) return
...
<template>
  <div class="game-page space-theme">
    ...
    <div v-if="isOnline" class="role-badge">{{ onlineRoleLabel }}</div>
...
.role-badge { position: relative; z-index: 1; margin-bottom: 4px; border: 1px solid rgba(56,189,248,0.24); border-radius: 999px; padding: 5px 12px; background: rgba(56,189,248,0.1); color: #bae6fd; font-size: 12px; font-weight: 700; }
