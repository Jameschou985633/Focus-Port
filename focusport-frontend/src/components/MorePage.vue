...
const settings = computed(() => [
  ...
  { icon: '🤖', title: 'AI 聊天', desc: '打开副官终端，进行学习规划和专注建议对话', action: () => router.push('/ai') },
...
function openMapWorld(code) {
  const targetDimension = 'PHYSICAL'
...
function handlePageWheel(event) {
  if (!pageRef.value || Math.abs(event.deltaY) <= Math.abs(event.deltaX)) return
  const nestedScroller = event.target?.closest?.('.modal-body, .profile-modal, .notification-modal, .password-modal')
  if (nestedScroller && nestedScroller !== pageRef.value && nestedScroller.scrollHeight > nestedScroller.clientHeight) {
    return
  }

  event.preventDefault()
  pageRef.value.scrollTop += event.deltaY
}
...
<template>
  <div ref="pageRef" class="more-page space-theme" @wheel="handlePageWheel">
...
.logout-btn :deep(button),
.logout-btn :deep(.space-button) {
  width: 100%;
}
