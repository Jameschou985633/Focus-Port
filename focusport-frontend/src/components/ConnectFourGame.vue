  ...
  if (msg.type === 'sync') {
    ...
    if (msg.moves) {
      for (const m of msg.moves) {
        if (m.row >= 0 && m.row < ROWS && m.col >= 0 && m.col < COLS) {
          board.value[m.row][m.col] = m.color
        }
      }
      const last = msg.last_move || msg.moves[msg.moves.length - 1]
      if (last) {
        const w = checkWinAt(last.row, last.col, last.color)
        if (w) {
          winCells.value = w
    ...
    if (!gameOver.value && board.value[0].every((_, c) => board.value[0][c] !== 0)) {
      isDraw.value = true
      gameOver.value = true
    ...
    if (msg.status === 'finished' || winner.value || isDraw.value) {
      gameOver.value = true
    }
