...
const isPhysical = computed(() => true)
const worldTitle = computed(() => '物理世界')
...
const applyRouteDimension = (username = currentUsername()) => {
  ...
  dimensionStore.setDimension('PHYSICAL', username)
  if (raw && !['PHYSICAL', '3D'].includes(String(raw).toUpperCase())) {
    router.replace({ path: '/island', query: { dimension: 'PHYSICAL' } })
  }
  return true
}

const switchDimension = (dimension) => {
  dimensionStore.setDimension('PHYSICAL', currentUsername())
}
...
watch(
  ...
  async () => {
    const username = currentUsername()
    applyRouteDimension(username)
    await inventoryStore.bootstrap(username)
  }
)
</script>
