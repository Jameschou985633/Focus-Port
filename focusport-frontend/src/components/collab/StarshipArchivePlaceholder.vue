...
const archiveEntries = computed(() => {
  ...
  return props.rooms.slice(0, 6).map((room, index) => {
    ...
    return {
      ...
      commander: room.owner_username || room.owner || room.name || `指挥官 ${index + 1}`,
      berth: occupied > 0 ? '已接入当前星环' : '等待接入',
      progress,
      note: room.description || '协作舱遥测等待上传。'
    }
  })
})
</script>
...
<template>
  <section class="archive-shell">
    <header class="archive-header">
      <div>
        <p class="archive-kicker">{{ WORLD_NAMES.starshipArchive.zh }}</p>
        <h2>{{ WORLD_NAMES.starshipArchive.zh }}</h2>
      </div>
      <span class="archive-chip">{{ archiveEntries.length }} 条信号</span>
    </header>
    ...
    <div v-if="archiveEntries.length" class="archive-grid">
      <article v-for="entry in archiveEntries" :key="entry.id" class="archive-card">
        ...
        <small>任务进度 {{ entry.progress }}%</small>
      </article>
    </div>

    <div v-else class="archive-empty">
      暂无协作舱遥测。创建公开协作舱后，这里会自动生成档案。
    </div>
  </section>
</template>
