...
const themes = [
  { value: 'space', label: '深空港', icon: '轨道' },
  { value: 'nebula', label: '星云泊位', icon: '星云' },
  { value: 'mars', label: '火星中继站', icon: '火星' },
...
<template>
  <div class="fleet-page">
    ...
    <section class="hero-panel">
      <div class="hero-copy">
        <p class="hero-kicker">{{ WORLD_NAMES.fleetNexus.zh }}</p>
        <h1>{{ WORLD_NAMES.fleetNexus.zh }}</h1>
        <p>
          与其他指挥官一起入座专注，查看实时协作舱，并把你的专注状态同步到共享星环。
        </p>
      </div>
    ...
    <section v-show="activeTab === 'rooms'" class="rooms-shell">
      ...
      <div v-if="!isLoading && rooms.length" class="search-bar">
        <input v-model="searchQuery" type="text" placeholder="按名称、主题或指挥官搜索协作舱..." class="space-input">
      </div>
      ...
      <div v-else-if="filteredRooms.length" class="rooms-grid">
        <SpaceCard
          ...
          :key="room.id"
          :title="room.name"
          :subtitle="room.description || '协作简报频道已开启。'"
    ...
    <SpaceModal v-model:visible="showCreateModal" :show-close="false" :title="WORLD_NAMES.fleetNexus.zh" width="460px">
      <div class="form-group">
        <label>协作舱名称</label>
        <input v-model="newRoom.name" type="text" maxlength="20" placeholder="清晨微积分小队" class="space-input">
      </div>

      <div class="form-group">
        <label>任务简报</label>
        <input v-model="newRoom.description" type="text" maxlength="50" placeholder="可选的协作说明" class="space-input">
      </div>

      <div class="form-group">
        <label>座位容量</label>
        <select v-model="newRoom.max_seats" class="space-select">
          <option :value="2">2 个座位</option>
          <option :value="4">4 个座位</option>
          <option :value="6">6 个座位</option>
          <option :value="8">8 个座位</option>
        </select>
      </div>
      ...
      <div v-if="!newRoom.is_public" class="form-group">
        <label>访问密钥</label>
        <input v-model="newRoom.password" type="password" placeholder="私密协作舱密码" class="space-input">
      </div>

      <p v-if="errorMessage" class="error-msg">{{ errorMessage }}</p>

      <div class="modal-actions">
        <SpaceButton variant="secondary" @click="showCreateModal = false">取消</SpaceButton>
