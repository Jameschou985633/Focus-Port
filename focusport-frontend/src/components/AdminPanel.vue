...
const api = axios.create({ baseURL: import.meta.env.VITE_API_BASE_URL || (import.meta.env.PROD ? '' : 'http://127.0.0.1:8010') })
...
const canNestedElementScroll = (element, deltaY) => {
...
const shouldUseNestedScroll = (target, deltaY) => {
  let element = target instanceof Element ? target : null
  while (element && element !== document.body && element !== document.documentElement) {
    if (canNestedElementScroll(element, deltaY)) return true
    element = element.parentElement
  }
  return false
}

const handleAdminWheel = (event) => {
  if (event.ctrlKey) return

  const deltaY = normalizeWheelDelta(event)
  if (!deltaY || shouldUseNestedScroll(event.target, deltaY)) return
...
onMounted(async () => {
  ...
  window.addEventListener('wheel', handleAdminWheel, { capture: true, passive: false })
...
onUnmounted(() => {
  window.removeEventListener('wheel', handleAdminWheel, true)
