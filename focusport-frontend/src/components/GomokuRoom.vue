...
function handleArcadeMessage(msg) {
  if (msg.type === 'sync') {
    ...
    lastMove.value = msg.last_move ? { row: msg.last_move.row, col: msg.last_move.col } : null
    winLine.value = []
    ...
    if (msg.moves) {
      ...
      if (last && checkWin(last.row, last.col, last.color)) {
...
function placeStone(row, col) {
  ...
  if (route.params.roomCode && pendingOnlineMove.value) return
  ...
  if (isOnline.value) {
    ...
    if (route.params.roomCode) pendingOnlineMove.value = true
    ws.send(JSON.stringify(payload))
    if (route.params.roomCode) return
    return
  }
