...
const onlineStep = ref('')
const roomCode = ref('')
const joinCode = ref('')
const joinError = ref('')
const createError = ref('')
const userCoins = ref(0)
const ONLINE_ROOM_COST = 25

const games = [
  { id: 'gomoku', name: '五子棋', icon: '●', desc: '经典五子连珠' },
  { id: 'tictactoe', name: '井字棋', icon: '×', desc: '3x3 经典对弈' },
  { id: 'connect4', name: '四子棋', icon: '●', desc: '纵向落子，四子连线' },
  { id: 'reversi', name: '黑白棋', icon: '◐', desc: '翻转对手棋子占领棋盘' },
  { id: 'doudizhu', name: '斗地主', icon: '♠', desc: '三人纸牌对战，抢地主后出牌压制' }
]
...
async function createRoom() {
  ...
  if (userCoins.value < ONLINE_ROOM_COST) {
    createError.value = `CU 不足，需要 ${ONLINE_ROOM_COST} CU，当前 ${userCoins.value} CU`
    return
  }
  ...
  try {
    ...
    createError.value = detail?.includes('CU') || detail?.includes('金币不足') ? `CU 不足，需要 ${ONLINE_ROOM_COST} CU` : detail || '创建房间失败'
  }
}
        ...
        <div class="modal-body">
          <div class="mode-buttons vertical">
            <button class="mode-btn wide" @click="createRoom">
              <span class="mode-icon">建</span>
              <span>创建房间</span>
              <span class="cu-cost">花费 {{ ONLINE_ROOM_COST }} CU</span>
            </button>
            <button class="mode-btn wide" @click="onlineStep = 'join'">
              <span class="mode-icon">入</span>
              <span>加入房间</span>
