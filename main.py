...
ADMIN_UPLOAD_ALLOWED_SUFFIXES = {".pdf", ".doc", ".docx", ".mp3", ".wav", ".m4a", ".ogg"}
FRONTEND_DIST = BASE_DIR / "focusport-frontend" / "dist"
FRONTEND_PUBLIC = BASE_DIR / "focusport-frontend" / "public"
CITY_ASSET_ROOT = FRONTEND_PUBLIC / "city-assets"
...
COMMERCIAL_MODEL_ROOT = CITY_ASSET_ROOT / "commercial" / "Models" / "GLB format"
COMMERCIAL_PREVIEW_BASE = "/city-assets/commercial/Previews"
COMMERCIAL_MODEL_BASE = "/city-assets/commercial/Models/GLB format"
DEFAULT_AVATAR = "👨‍🚀"
...
def is_commercial_building_model(path: Path) -> bool:
    name = path.name.lower()
    return path.suffix.lower() == ".glb" and (
        name.startswith("building-") or
        name.startswith("low-detail-building-")
    )


def commercial_asset_name_cn(stem: str) -> str:
...
def sync_commercial_city_items(conn: sqlite3.Connection) -> None:
    if not COMMERCIAL_MODEL_ROOT.exists():
        return

    model_files = sorted(
        [path for path in COMMERCIAL_MODEL_ROOT.iterdir() if is_commercial_building_model(path)],
    ...
    for index, model_file in enumerate(model_files):
        ...
        payload = (
            ...
            "🏢",
            0,
            commercial_asset_rarity(stem),
            f"{name_cn}，可部署到 3D 物理视界实体基地。",
            1,
            3000 + index,
            commercial_asset_price(stem),
            "3D",
            preview_path,
            preview_path,
            1,
...
def disable_imported_asset_items(conn: sqlite3.Connection) -> None:
    legacy_vendor_tag = "ke" + "nney"
    conn.execute(
        """
        UPDATE Unified_Shop_Items
        SET is_available = 0
        WHERE item_code LIKE 'physical_%'
           OR item_code LIKE 'city_%'
           OR LOWER(COALESCE(tags, '')) LIKE ?
           OR model_path LIKE '/models/%'
           OR model_path LIKE '/city-assets/%'
        """,
        (f"%{legacy_vendor_tag}%",),
    )
...
def sync_runtime_shop_catalog(conn: sqlite3.Connection) -> None:
    ...
    disable_imported_asset_items(conn)
    sync_commercial_city_items(conn)
    conn.commit()
...
def ensure_user(conn: sqlite3.Connection, username: str, password: str = "123456") -> None:
    ...
    elif username == ADMIN_TEST_USERNAME:
        conn.execute("UPDATE Users SET password = ? WHERE username = ?", (ADMIN_TEST_PASSWORD, username))
    ...
    if username == ADMIN_TEST_USERNAME:
        conn.execute(
            "UPDATE User_Growth SET coins = ?, diamonds = MAX(COALESCE(diamonds, 0), 999999), sunshine = MAX(COALESCE(sunshine, 0), 999999) WHERE username = ?",
            (ADMIN_TEST_COMPUTE, username),
        )
...
def build_growth_payload(conn: sqlite3.Connection, username: str) -> dict[str, Any]:
    ...
    return {
        ...
        "coins": ADMIN_TEST_COMPUTE if username == ADMIN_TEST_USERNAME else int(row["coins"] or 0),
...
def add_currency(conn: sqlite3.Connection, username: str, coins: int = 0, diamonds: int = 0, source: str = "") -> None:
    ...
    if coins:
        ...
        conn.execute(
            "INSERT INTO Sunshine_Transactions (username, amount, transaction_type, source, description) VALUES (?, ?, 'compute_gain', ?, ?)",
            (username, coins, source or "system", f"Compute gain from {source or 'system'}"),
        )
    if diamonds:
...
def award_daily_pomodoro_compute(conn: sqlite3.Connection, username: str) -> dict[str, Any]:
    ensure_user(conn, username)
    today_key = date.today().isoformat()
    reward_key = f"daily_pomodoro_compute:{username}:{today_key}"
    existing = conn.execute("SELECT value FROM Runtime_Metadata WHERE key = ?", (reward_key,)).fetchone()
    if existing:
        return {"awarded": False, "amount": 0, "date": today_key}

    add_currency(conn, username, coins=DAILY_POMODORO_COMPUTE_REWARD, source="daily_pomodoro_compute")
    conn.execute(
        """
        INSERT OR REPLACE INTO Runtime_Metadata (key, value, updated_at)
        VALUES (?, ?, CURRENT_TIMESTAMP)
        """,
        (reward_key, json.dumps({"username": username, "date": today_key, "amount": DAILY_POMODORO_COMPUTE_REWARD}, ensure_ascii=False)),
    )
    return {"awarded": True, "amount": DAILY_POMODORO_COMPUTE_REWARD, "date": today_key}
...
def spend_currency(conn: sqlite3.Connection, username: str, coins: int = 0, diamonds: int = 0, source: str = "") -> None:
    ...
    if coins and username == ADMIN_TEST_USERNAME:
        ...
        conn.execute(
            "INSERT INTO Sunshine_Transactions (username, amount, transaction_type, source, description) VALUES (?, ?, 'compute_spend', ?, ?)",
            (username, 0, source or "system", f"Admin test compute spend bypass for {source or 'system'}"),
        )
    elif coins:
        conn.execute("UPDATE User_Growth SET coins = coins - ? WHERE username = ?", (coins, username))
        conn.execute(
            "INSERT INTO Sunshine_Transactions (username, amount, transaction_type, source, description) VALUES (?, ?, 'compute_spend', ?, ?)",
            (username, -coins, source or "system", f"Compute spend for {source or 'system'}"),
        )
    if diamonds:
...
def award_focus_completion(conn: sqlite3.Connection, username: str, duration: int, subject: str) -> dict[str, Any]:
    ...
    daily_reward = award_daily_pomodoro_compute(conn, username)
    growth = build_growth_payload(conn, username)
    return {"success": True, "message": "专注记录已同步", "exp_gained": exp_gain, "coins_gained": coins_gain, "diamonds_gained": diamonds_gain, "daily_pomodoro_compute_reward": daily_reward, "growth": growth}
...
def award_focus_completion_v2(
    ...
    return {
        ...
        "daily_pomodoro_compute_reward": daily_reward,
...
def generate_ai_reply(conn: sqlite3.Connection, username: str, message: str) -> str:
    ...
    if any(key in message for key in ["你好", "hello", "hi", "在吗", "哈喽"]):
        return "\n".join([
            "我在。你可以直接问我三类问题：",
    ...
    if any(key in message for key in ["怎么使用", "如何使用", "怎么用", "使用这个网站", "使用网站", "这个网站", "新手", "教程", "功能介绍"]):
        return "\n".join([
            "FocusPort 可以按这个顺序用：",
    ...
    if any(key in message for key in ["计划", "安排", "节奏", "今晚", "今天", "明天", "日程", "规划"]) or "plan" in lowered:
        return "\n".join([
            snapshot,
            "",
            "我建议这样安排：",
            "1. 先用 5-10 分钟整理桌面和资料，只保留一个任务入口。",
            "2. 开一轮 25 分钟专注，优先处理最难或最拖延的任务。",
            "3. 休息 5 分钟，不刷娱乐内容。",
            "4. 再开一轮 25 分钟，把结果整理成可提交或可复盘的版本。",
            "5. 最后写三句话：完成了什么、卡在哪里、下一步做什么。",
            "",
            f"优先处理这些待办：\n{task_lines}",
        ])
    if any(key in message for key in ["分心", "专注", "拖延", "效率", "学不进去", "坐不住"]) or "focus" in lowered:
        return "\n".join([
            snapshot,
            "",
            "你现在需要把入口变窄：手机放远、桌面只留一个任务、先做 15 分钟热启动。",
            "前 5 分钟只要求进入状态，不追求效率；后 10 分钟完成一个看得见的小产出。",
            "",
            f"现在可以从这里开始：\n{tasks[0] if tasks else '新建一个最小任务，然后立刻开 15 分钟。'}",
        ])
    if any(key in message for key in ["薄弱", "弱点", "提分", "分析", "复盘", "状态", "问题在哪"]):
        return "\n".join([
            snapshot,
            "",
            "我会先从三点判断薄弱处：任务完成率、专注持续性、复盘质量。",
            "今天的改法：选一个最容易丢分或最拖延的点，做 25 分钟专项训练，然后把错因归成“不会、粗心、时间不够、知识点混淆”四类。",
        ])
    if any(key in message for key in ["考试", "英语", "真题", "错题", "阅读", "听力", "写作", "词汇"]):
        return "\n".join([
            snapshot,
            "",
            "复习建议按“限时训练-错因分类-二次复查”走，不要只刷题。",
            "1. 先做一组限时题。",
            "2. 标出错因：知识点、审题、速度、表达。",
            "3. 把错题改写成 3 条可复习的问题。",
            "4. 明天只复查这些问题。",
        ])
    if any(key in message for key in ["动力", "坚持", "焦虑", "压力", "累", "不想学", "没动力", "崩"]):
        return "\n".join([
            snapshot,
            "",
            "先别逼自己热血起来，把目标降到足够小：只承诺 10 分钟。",
            "开始前写一句“这 10 分钟只完成什么”，做完马上勾掉任务或记录专注，让系统给你一个反馈。",
            f"你已经累计了 {growth['total_focus_minutes']} 分钟专注，下一步不是重来，是续上。",
        ])
    if any(key in message for key in ["项目", "代码", "部署", "render", "github", "后端", "前端", "README", "答辩", "提交"]):
        return "\n".join([
    ...
    if any(key in message for key in ["cu", "算力", "购买", "商城", "物品", "金币", "奖励"]):
        return "\n".join([
    ...
    if any(key in message for key in ["好友", "社交", "加好友", "请求", "朋友"]):
        return "\n".join([
            "社交功能的基本流程是：搜索对方用户名，发送好友请求，对方账号收到后确认，双方才会成为好友。",
            "如果你在测试，请用两个不同账号分别登录验证：A 发请求，B 同意，A 再刷新好友列表。",
            "不要用同一个账号测试收发双方，否则无法模拟真实好友确认流程。",
        ])
    if any(key in message for key in ["手机", "截图", "屏幕时间", "屏幕使用", "自律", "娱乐时间"]):
        return "\n".join([
...
async def admin_upload_resource(file: UploadFile = File(...)) -> dict[str, Any]:
    filename = safe_admin_upload_filename(file.filename or "")
    ...
    return {"success": True, "message": "上传成功", "filename": filename, "url": f"/static/{filename}"}
...
def get_stats(username: str, period: str = "week") -> dict[str, Any]:
    ...
    with closing(get_conn()) as conn:
        ...
        total_focus_minutes = sum(day["focus_minutes"] for day in daily_stats)
        legacy_focus_minutes = int(growth.get("total_focus_minutes", 0) or 0)
        if total_focus_minutes == 0 and legacy_focus_minutes > 0 and daily_stats:
            daily_stats[-1]["focus_minutes"] = legacy_focus_minutes
            total_focus_minutes = legacy_focus_minutes
        summary = {
            "totalFocusHours": round(total_focus_minutes / 60.0, 1),
...
def unified_shop_place(payload: UnifiedShopPlaceRequest) -> dict[str, Any]:
    with closing(get_conn()) as conn:
        ...
        if target_dimension != item_dimension:
            raise HTTPException(status_code=400, detail="物件维度与当前部署区域不匹配")
        if target_dimension == "3D" and payload.map_id == "city":
            ...
            if grid_x is None or grid_y is None:
                raise HTTPException(status_code=400, detail="2D 部署必须提供网格坐标")
            ...
            if conflict:
                raise HTTPException(status_code=400, detail=f"{conflict['name_cn'] or conflict['name']} 已占用这片网格")
            pos_x = float(grid_x)
...
@app.get("/api/compute-ledger/{username}")
def compute_ledger(username: str, limit: int = 80) -> dict[str, Any]:
    safe_limit = max(1, min(limit, 200))
    with closing(get_conn()) as conn:
        rows = conn.execute(
            """
            SELECT amount, transaction_type, source, description, created_at
            FROM Sunshine_Transactions
            WHERE username = ?
              AND transaction_type IN ('compute_gain', 'compute_spend')
            ORDER BY id DESC
            LIMIT ?
            """,
            (username, safe_limit),
...
def _normalize_arcade_game(game: str) -> str:
    return str(game or "").removesuffix("_online")


def _arcade_expected_player(room: dict[str, Any]) -> str | None:
    return room.get("player_host") if int(room.get("current_turn") or 1) == 1 else room.get("player_guest")


def _arcade_expected_color(room: dict[str, Any]) -> int:
    return 2 if int(room.get("current_turn") or 1) == 2 else 1


def _arcade_cell_occupied(room: dict[str, Any], row: int, col: int) -> bool:
    return any(int(move.get("row", -1)) == row and int(move.get("col", -1)) == col for move in room.get("moves") or [])


def _arcade_game_type(room: dict[str, Any]) -> str:
...
def safe_admin_upload_filename(filename: str) -> str:
    ...
    if suffix not in ADMIN_UPLOAD_ALLOWED_SUFFIXES:
        raise HTTPException(status_code=400, detail="仅支持 PDF / Word / 音频文件")
    safe_stem = re.sub(r"[^\w\u4e00-\u9fff.-]+", "_", stem, flags=re.UNICODE).strip("._")
    if not safe_stem:
        safe_stem = f"resource_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
    return f"{safe_stem}{suffix}"


def _arcade_winner_username(room: dict[str, Any]) -> str | None:
    winner = int(room.get("winner") or 0)
    if not winner:
        return None
    players = _arcade_players(room)
    ...
    if winner == int(room.get("guest_color") or 2):
        return room.get("player_guest")
    return None


def _award_arcade_winner_once(room: dict[str, Any], source: str) -> None:
    if room.get("winner_rewarded"):
        return
...
async def arcade_join(payload: ArcadeJoinPayload) -> dict[str, Any]:
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "state":
                ...
                expected_player = _arcade_expected_player(room)
                if not expected_player or username != expected_player:
                    await websocket.send_json({"type": "error", "detail": "还没轮到你。"})
                    continue
                ...
                room["status"] = "finished"
                room["winner"] = int(msg.get("winner") or msg.get("winner_color") or 0)
                room["is_draw"] = not room["winner"]
                if room["winner"]:
                    _award_arcade_winner_once(room, f"arcade_win:{_arcade_game_type(room)}")
                for ws_conn in list(_arcade_ws_connections.get(room_code, set())):
                    try:
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "move":
                ...
                game["status"] = "finished"
                game["winner"] = msg.get("winner_color")
